...
async fn recover_open_returns_only_non_terminal_orders() {
    ...
    let recovered = recover_open(fake.as_ref(), keys.as_ref())
        .await
        .expect("recover");
    assert_eq!(recovered.len(), 1);
    assert_eq!(recovered[0].order.id, open.order.id);
}
...
async fn recover_open_repairs_a_lagging_object() {
    ...
    let recovered = recover_open(fake.as_ref(), keys.as_ref())
        ...
        .expect("recover");
    assert_eq!(recovered.len(), 1);
    assert_eq!(recovered[0], candidate);
...
async fn recover_open_never_double_applies_a_fill() {
    ...
    let recovered = recover_open(fake.as_ref(), keys.as_ref())
        ...
        .expect("recover");
    assert_eq!(recovered.len(), 1);
    assert_eq!(recovered[0].filled_input, AtomicAmount::new(400));
    assert_eq!(recovered[0].order.remaining_input, AtomicAmount::new(600));
    assert!(conservation_holds(&recovered[0]));
}
...
async fn recover_open_repairs_terminal_orders_without_returning_them() {
    ...
    let recovered = recover_open(fake.as_ref(), keys.as_ref())
        .await
        .expect("recover");
    assert!(recovered.is_empty(), "terminal orders are not open");
    assert_eq!(
        fake.latest_object(&object_id).map(|object| object.version),
        Some(filled.version)
    );
...
#[tokio::test]
async fn recover_open_fails_closed_on_a_tampered_object() {
    let fake = fake();
    let keys = keys();
    let store = durable_store(&fake, &keys);
    let order = durable_order(&keys, "rec-tamper", OrderStatus::Created, 1_000, 1_000, 0);
    store.create(order.clone()).await.expect("create");

    let mut object = fake.objects().into_iter().next().expect("object");
    let last = object.ciphertext.len() - 1;
    object.ciphertext[last] ^= 0xFF;
    fake.replace_object(object);

    assert_eq!(
        recover_open(fake.as_ref(), keys.as_ref()).await,
