//! Deterministic unit tests for sell-side transfer-tax input arithmetic.
...
fn sample_gross_input(chain: ChainId, address: &str, amount: u128) -> AssetAmount {
    AssetAmount {
...
fn test_floor_rounding_vectors_and_conservation() {
    ...
    // (gross_amount, sell_tax_bps, expected_tax, expected_net)
    ...
    for &(gross, bps, exp_tax, exp_net) in vectors {
        ...
        let gross_input = sample_gross_input(ChainId::Base, address, gross);

        let result = apply_sell_tax_to_input(&assessment, &gross_input)
            .unwrap_or_else(|e| panic!("failed for gross={}, bps={}: {:?}", gross, bps, e));

        assert_eq!(
            result.gross_input().amount.get(),
            gross,
            "gross input mismatch"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            exp_net,
            "net transferable input mismatch for gross={}, bps={}",
            gross,
            bps
        );
        assert_eq!(
            result.net_input().amount.get(),
            exp_net,
            "net input alias mismatch"
        );
        ...
        assert_eq!(result.gross_input().asset, gross_input.asset);
        assert_eq!(result.tax_cost().asset, gross_input.asset);
        assert_eq!(result.net_transferable_input().asset, gross_input.asset);

        // Verify convenience callers produce identical results
        let res_alias = apply_sell_tax(&assessment, &gross_input).unwrap();
        assert_eq!(result, res_alias);

        let res_calc = calculate_sell_tax_input(&assessment, &gross_input).unwrap();
        assert_eq!(result, res_calc);

        let res_engine = TaxSafetyEngine::apply_sell_tax(&assessment, &gross_input).unwrap();
        assert_eq!(result, res_engine);

        let res_engine_to =
            TaxSafetyEngine::apply_sell_tax_to_input(&assessment, &gross_input).unwrap();
        assert_eq!(result, res_engine_to);

        let res_method = assessment.apply_sell_tax(&gross_input).unwrap();
        assert_eq!(result, res_method);

        let res_method_to = assessment.apply_sell_tax_to_input(&gross_input).unwrap();
        assert_eq!(result, res_method_to);
    }
}
...
fn test_zero_tax_success() {
    ...
    for gross in [1u128, 100, 10_000, 1_000_000_000, u128::MAX] {
        ...
        assert_eq!(result.net_transferable_input().amount.get(), gross);
        assert_eq!(
            result.gross_input().amount.get(),
            result.tax_cost().amount.get() + result.net_transferable_input().amount.get()
        );
...
fn test_100_percent_tax_zero_net_rejection() {
    ...
    for gross in [1u128, 2, 100, 10_000, 1_000_000_000, u128::MAX] {
        let gross_input = sample_gross_input(ChainId::Base, address, gross);
        let err = apply_sell_tax_to_input(&assessment, &gross_input)
            .expect_err("100% tax must fail closed with ZeroNetInput");

        assert_eq!(err, TaxSafetyError::ZeroNetInput);
    }
}
...
fn test_near_u128_max_gross_exact_bounded_arithmetic() {
    ...
    for &gross in &test_gross_amounts {
        for &bps in test_bps_rates {
            ...
            let result = apply_sell_tax_to_input(&assessment, &gross_input).unwrap_or_else(|e| {
                panic!("panic or error for gross={}, bps={}: {:?}", gross, bps, e)
            });

            // Conservation must hold exactly
            let tax = result.tax_cost().amount.get();
            let net = result.net_transferable_input().amount.get();
            assert_eq!(
                tax.checked_add(net),
                Some(gross),
        ...
        // 10_000 bps (100%) on near-u128::MAX must reject fail closed with ZeroNetInput
        let assessment_100pct =
            sample_assessment(ChainId::Base, address, 10_000, FreshnessStatus::Fresh);
        let gross_input = sample_gross_input(ChainId::Base, address, gross);
        let err = apply_sell_tax_to_input(&assessment_100pct, &gross_input)
            .expect_err("100% tax on near-u128::MAX must reject with ZeroNetInput");
        assert_eq!(err, TaxSafetyError::ZeroNetInput);
    }
}
...
fn test_stale_assessment_rejection_and_immutability() {
    ...
    let gross_input = sample_gross_input(ChainId::Base, address, 1_000_000);

    let assessment_before = assessment.clone();
    let gross_input_before = gross_input.clone();
    ...
    assert_eq!(gross_input, gross_input_before, "gross_input was mutated");
}
...
fn test_chain_mismatch_rejection_and_immutability() {
    ...
    // Provide gross input on Ethereum instead of Base
    let gross_input = sample_gross_input(ChainId::Ethereum, address, 1_000_000);
...
fn test_assessed_asset_mismatch_rejection_and_immutability() {
    ...
    // Provide gross input with a different token address on the same chain
    let gross_input = sample_gross_input(ChainId::Base, address_b, 1_000_000);
...
fn test_zero_gross_amount_rejection_and_immutability() {
    let address = "0x0000000000000000000000000000000000000002";
    let assessment = sample_assessment(ChainId::Base, address, 300, FreshnessStatus::Fresh);
    let gross_input = sample_gross_input(ChainId::Base, address, 0);
    ...
    assert_eq!(err, TaxSafetyError::ZeroGrossInput);
    assert_eq!(assessment, assessment_before, "assessment was mutated");
    assert_eq!(gross_input, gross_input_before, "gross_input was mutated");
}
...
fn test_success_case_input_immutability() {
    ...
    let res = apply_sell_tax_to_input(&assessment, &gross_input).expect("valid sell tax");
    assert_eq!(res.tax_cost().amount.get(), 50_000);
    assert_eq!(res.net_transferable_input().amount.get(), 950_000);
    ...
    assert_eq!(
        gross_input, gross_input_before,
        "gross_input was mutated on success"
    );
}
...
fn test_redaction_regression_mismatch_errors() {
    ...
    let gross_input = sample_gross_input(ChainId::Base, raw_addr_observed, 1_000_000);

    let err = apply_sell_tax_to_input(&assessment, &gross_input).unwrap_err();
    assert_eq!(err, TaxSafetyError::AssessedAssetMismatch);
    ...
    // Chain mismatch
    let gross_input_chain = sample_gross_input(
        ChainId::Other("custom-secret-chain-12345".to_string()),
        raw_addr_expected,
        1_000_000,
    );
    let err_chain = apply_sell_tax_to_input(&assessment, &gross_input_chain).unwrap_err();
...
fn test_redaction_regression_stale_and_resync_errors() {
    ...
    let gross_input = sample_gross_input(ChainId::Base, distinct_address, 1_000_000);

    let err_stale = apply_sell_tax_to_input(&assessment_stale, &gross_input).unwrap_err();
    ...
    let err_resync = apply_sell_tax_to_input(&assessment_resync, &gross_input).unwrap_err();
...
fn test_redaction_regression_zero_net_and_zero_gross_errors() {
    ...
    // 1. Zero net input (100% tax on distinct gross amount)
    ...
    let gross_input = sample_gross_input(ChainId::Base, distinct_address, distinct_gross);

    let err_zero_net = apply_sell_tax_to_input(&assessment_100, &gross_input).unwrap_err();
    assert_eq!(err_zero_net, TaxSafetyError::ZeroNetInput);
    ...
    assert_eq!(
        zero_net_display,
        "net transferable input amount must be greater than zero"
    );
    assert_eq!(zero_net_debug, "ZeroNetInput");
    ...
    let gross_zero = sample_gross_input(ChainId::Base, distinct_address, 0);

    let err_zero_gross = apply_sell_tax_to_input(&assessment_normal, &gross_zero).unwrap_err();
    assert_eq!(err_zero_gross, TaxSafetyError::ZeroGrossInput);
    ...
    assert_eq!(
        zero_gross_display,
        "gross input amount must be greater than zero"
    );
    assert_eq!(zero_gross_debug, "ZeroGrossInput");
...
#[test]
fn test_sell_tax_input_serde_round_trip() {
    let address = "0x0000000000000000000000000000000000000002";
    let assessment = sample_assessment(ChainId::Base, address, 250, FreshnessStatus::Fresh);
    let gross_input = sample_gross_input(ChainId::Base, address, 1_000_000);

    let result: SellInputTax = apply_sell_tax_to_input(&assessment, &gross_input).unwrap();

    let json = serde_json::to_string(&result).expect("serialize SellTaxInput");
    let deserialized: SellTaxInput = serde_json::from_str(&json).expect("deserialize SellTaxInput");
